  ...
  export async function guardarResultado(req, res) {
    try {
      let { nombre, puntaje, nivel } = req.body;

      nombre = typeof nombre === 'string' && nombre.trim() !== '' ? nombre.trim() : 'Anónimo';

      
      if (typeof puntaje !== 'number' || typeof nivel !== 'number') {
        return res.status(400).json({ error: 'Datos inválidos o incompletos' });
      }

      const [resultado] = await connection.query(
        'INSERT INTO resultados (nombre, puntaje, nivel) VALUES (?, ?, ?)',
  ...
  export async function obtenerResultados(req, res) {
    try {
      const [resultados] = await connection.query(
        'SELECT nombre, puntaje, nivel, fecha FROM resultados ORDER BY puntaje DESC, fecha DESC LIMIT 10'
      );
